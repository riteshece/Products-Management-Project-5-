...
const createOrder = async function (req,res){
    try{
        ...
        return res.status(400).send({status:false,msg:"order details is must be require in request body"})
        }
...
const updateOrder = async function(req,res) {
    try{
        ...
        if(!userSearch) {
            return res.status(400).send({status: false, msg: "userId does not exist in db"})
        }
