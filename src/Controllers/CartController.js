...
const createCart = async function(req,res) {
    try {
        ...
        if(!(userId)) {
         return res.status(400).send({ status: false, msg: "UserId is require in param"});
            }


        const userSearch = await userModel.findById({_id: userId})
        if(!userSearch) {
            return res.status(400).send({status: false, msg: "User does not exist in db"})
        }
...
const deleteCart = async function(req,res) {
    try{
        ...
        res.status(200).send({status: true, msg:"Cart successfully deleted",data:cartDelete})
